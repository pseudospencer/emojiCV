...
function currentEmojiOverFace() {
    // Draw a big emoji with the current emotion that tracks and covers the face
    let centerX = positions[62][0];
    let centerY = positions[62][1];
    let faceRadius = dist(positions[62][0], positions[62][1], positions[7][0], positions[7][1]); // nose tip xy, chin tip xy

    textSize(faceRadius * 2);
    textAlign(CENTER, CENTER);
    text(lastEmotions[0], centerX, centerY);
}
